...
export default function Edit({ category }) {
    ...
    const handleSubmit = (e) => {
        ...
        put(route('admin.blog-categories.update', category.id), {
            onSuccess: () => {
                if (window.toast) {
                    window.toast.success('Kategori Blog Berhasil Diperbarui', 'Data kategori blog telah diperbarui.');
                }
            },
            onError: () => {
                if (window.toast) {
                    window.toast.danger('Gagal Memperbarui Kategori Blog', 'Silakan periksa kembali inputan form.');
                }
            },
        });
    };
    ...
    return (
        <AdminLayout title={`Edit Blog Category - ${category?.name}`}>
            <Head title={`Edit Category: ${category?.name}`} />
            <PageHeader title="Edit Blog Category" pretitle="Content Management" />
